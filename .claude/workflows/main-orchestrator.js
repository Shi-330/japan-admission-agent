...
for (const sprint of planResult.sprints) {
  ...
  for (let retry = 1; retry <= MAX_RETRIES; retry++) {
    ...
    if (evalResult.score >= PASS_THRESHOLD) {
      ...
      passed = true;
      // Commit + push: only push verified code
      const commitMsg = `sprint(${featureIndex}): ${sprintName} (score ${evalResult.score}/100)`;
      await agent(
        `Sprint ${featureIndex} passed evaluation at ${evalResult.score}/100. Commit all changes and push to origin/main.\n` +
        `Commit message: "${commitMsg}"\n` +
        `Run: git add -A && git commit -m "${commitMsg}" && git push origin HEAD:main`,
        { label: `push:${featureIndex}` }
      );
  ...
  if (!passed) {
    log(`⚠️  Sprint "${sprintName}" FAILED after ${MAX_RETRIES} retries — rolling back to clean state`);
    // Rollback: reset to tag, clean all junk
    await agent(
      `Sprint ${featureIndex} failed all retries. Rollback:\n` +
      `1. git checkout sprint-${featureIndex}\n` +
      `2. git reset --hard pre-build-sprint-${featureIndex}\n` +
      `3. git tag -d pre-build-sprint-${featureIndex}\n` +
      `4. git clean -fd\n` +
      `This restores the repo to exactly the state before Builder started.`,
      { label: `rollback:${featureIndex}` }
    );
