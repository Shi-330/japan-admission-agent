...
import { normalize as cn2jp_normalize } from '@/lib/cn2jp';
...
export default function PlazaView({ catalog, stage, token, apiCall, setStage, showToast, profile }) {
  ...
  if (!filter) {
    ...
    const terms = cn2jp_normalize(filter);
    list = list.filter(s => terms.some(t =>
      [s.name, ...(s.majors||[]), ...(s.tags||[]), s.notes||''].join(' ').toLowerCase().includes(t.toLowerCase())
    ));
  }
